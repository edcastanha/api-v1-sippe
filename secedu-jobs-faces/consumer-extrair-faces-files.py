...
class ConsumerExtractor:
    def __init__(self):
        ...
        logger.debug(f' <**_**> ConsumerExtractor: INIT')
    ...
    def process_message(self, ch, method, properties, body):
        ...
        device = data['local']
        logger.debug(f' <**_**> ConsumerExtractor: process_message:: {data}')
        ...
        if file.lower().endswith(('.jpg', '.jpeg', '.png')):
            ...
            logger.debug(f' <**_**> ConsumerExtractor: process_message:: {message_dict}')
            ...
            try:
                for index, face_obj in enumerate(face_objs):
                    if face_obj['confidence'] >= LIMITE_DETECTOR:
                        ...
                        logger.debug(f' <**SAVE NEW FACE**> Path:: {save_path}: ')
                        ...
                        logger.debug(f' <**_ REDIS _**> PIPELINE :: {pipeline_results}')
            except Exception as e:
                logger.debug(f' <**EXTRATOR FACE**> Erro:: {e}')
